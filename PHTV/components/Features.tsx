...
const FeatureCard: React.FC<FeatureProps> = ({ icon: Icon, title, description, color = "text-brand-400" }) => (
  <div className="glass-panel p-8 rounded-[2rem] hover:bg-white/40 dark:hover:bg-white/5 transition-all duration-500 border border-slate-200/50 dark:border-white/5 hover:border-brand-500/20 group h-full flex flex-col relative overflow-hidden shadow-sm dark:shadow-none">
    ...
    <div className={`w-14 h-14 rounded-2xl bg-slate-100 dark:bg-slate-900/50 flex items-center justify-center mb-8 group-hover:scale-110 transition-transform duration-500 border border-slate-200 dark:border-white/5 ${color} shadow-lg`}>
      <Icon size={28} />
    </div>
    <h3 className="text-xl font-extrabold text-slate-900 dark:text-white mb-4 tracking-tight group-hover:text-brand-500 dark:group-hover:text-brand-400 transition-colors">{title}</h3>
    ...
    <section id="features" className="py-32 relative">
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-20">
          <h2 className="text-4xl md:text-6xl font-extrabold mb-6 bg-clip-text text-transparent bg-gradient-to-b from-slate-900 to-slate-500 dark:from-white dark:to-slate-500 tracking-tight">
            Sức mạnh vượt trội
          </h2>
